...
import sys
sys.path.append('../')
from library import parseXmlStopStemRem, findVocab
...
if os.path.exists("data/my_amazon_train.txt"):
    ## Open the file with read only permi

    f = codecs.open('data/my_amazon_train_VOCAB.txt', "r", encoding="utf-8")
    unique_words = [x.strip('\n') for x in f.readlines()]
    f.close()

    f = codecs.open('data/my_amazon_train.txt', "r", encoding="utf-8")
    train = [x.strip('\n') for x in f.readlines()]
    f.close()
    ...
    for path in paths:
        print path
        path_pos = 'data/'+path+'/data_train.txt'

        f = codecs.open(path_pos, "r", encoding="utf-8")
        train = [x.strip('\n') for x in f.readlines()]
        f.close()
        ...
        path_pos = 'data/'+path+'/y_train.txt'
        ...
        for y in y_train:
            y_train_all.append(int(y))

    unique_words = findVocab(train_data)

    f = codecs.open('data/my_amazon_train_VOCAB.txt', "w", encoding="utf-8")
    ...
    f = codecs.open('data/my_amazon_train.txt', "w", encoding="utf-8")
...
## Testing set
if os.path.exists("data/my_amazon_test.txt"):
    ## Open the file with read only permit
    f = codecs.open('data/my_amazon_test.txt', "r", encoding="utf-8")
    test = [x.strip('\n') for x in f.readlines()]
    f.close()
    ...
    for path in paths:
        print path
        path_pos = 'data/'+path+'/data_test.txt'
        ...
        for y in y_test:
            y_test_all.append(int(y))

    test_data = parseXmlStopStemRem(test_data,unique_words,bigrams,trigrams,False)


    f = codecs.open('data/my_amazon_test.txt', "w", encoding="utf-8")
...
if idf_bool:
    ...
    np.savetxt("tfidf_predictions_False.txt",y_test,fmt='%i')
